...
class App extends Component {
  ...
  componentDidMount = () => {
    const setInt = () => {
      const {timeLeft, intervalId, gameOver} = this.state
  ...
  onThumbnailClick = inp => {
    if (inp) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        randomImageIndex: Math.ceil(Math.random() * imagesList.length),
      }))
    } else {
  ...
  render() {
    const {score, gameOver, randomImageIndex, timeLeft, intervalId} = this.state

    const randomImageId = imagesList[randomImageIndex].id
    ...
    const game = (
      <div>
        <div className='mainSection'>
          <img alt='match' className='mainImage' src={randomImageUrl} />
        </div>
        <div className='menuSection'>
          <ul className='tabUl' type='none'>
        ...
        <div className='footerSection'>
          <ul className='thumbnailsUl' type='none'>
    ...
    const gameover = <GameOver onReset={this.onReset} score={score} />
    ...
    return (
      <div className='appComponent'>
        ...
        <div>{gameOver || timeLeft === 0 ? gameover : game}</div>
      </div>
    )
  }
}
